...
const METRICS = [
  ['NY.GDP.MKTP.KD.ZG','Crecimiento','PBI real · YOY'],
  ['FP.CPI.TOTL.ZG','Inflación','Precios al consumidor · YOY'],
...
export function createComparisonLab(ctx) {
  ...
  function mountLatam() {
    ...
    const html=`<section id="comparison-lens" class="comparison-lens" aria-labelledby="lens-title">
      <div class="lab-section-heading"><div><h2 id="lens-title">Perú y sus pares</h2><p>Comparación al mismo año. Mediana de pares sin ponderación.</p></div><span class="lab-coverage">${coverage} / ${allSeries.length} datos · ${dateLabel}</span></div>
      ${extra.length?`<p class="lab-notice">La selección incluye referentes fuera de Latinoamérica: ${extra.map(c=>esc(label(c))).join(', ')}.</p>`:''}
      <div id="lab-country-selection"><p class="lab-country-caption">Países</p></div>
      <div class="lab-controls"><label>Año<select id="lens-year">${optionYears(available,lensYear)}</select></label><label>Comparar desde<select id="lens-baseline"><option value="">Sin comparación</option>${optionYears(available.filter(d=>d<lensYear),baselineYear)}</select></label><button id="lens-common" ${common.length?'':'disabled'}>Último año completo</button></div>
      ${!common.length?'<p class="lab-notice">No hay un año con cobertura completa para esta selección.</p>':''}
      <div class="lens-cards">${METRICS.map(([code,title,definition],i)=>{
        ...
        return `<article class="lens-card" style="--lens-color:${colors[i%colors.length]}"><div class="lens-card-top"><h3>${title}</h3><span>${dateLabel}</span></div><p>${definition}</p><div class="lens-value"><strong>${p?fmt(p.value,isMoney?0:2):'—'}</strong><span>${isMoney?'US$':esc(pe?.unit||allSeries.find(s=>s?.sourceCode===code)?.unit||'')}</span></div><small>${countries.includes('PER')?'Perú':'Perú no está seleccionado'}</small>
          ...
          <div class="lens-change">${baselineYear?`${baselineYear.slice(0,4)} → ${dateLabel}: <b>${finite(delta)?signed(delta)+' '+esc(deltaUnit):'sin ambos datos'}</b>`:'Sin periodo de comparación'}</div>
          ${pe?`<button class="link-button" data-detail="${esc(pe.id)}"><i class="fa-solid fa-chart-line" aria-hidden="true"></i> Ver serie</button>`:''}</article>`;
      }).join('')}</div>
      <div class="lens-key"><span><i class="lens-key-peru"></i> Perú</span><span><i class="lens-key-peer"></i> Mediana de los demás países</span><span>Escala: mínimo–máximo de la selección</span></div>
      <details class="lab-matrix"><summary>Valores y variaciones por país</summary><div class="table-scroll"><table><caption>Banco Mundial · mismo año ${dateLabel}${baselineYear?' y cambio frente a '+baselineYear.slice(0,4):''}. Cada indicador conserva su propia unidad.</caption><thead><tr><th>Indicador / unidad</th>${countries.map(c=>`<th class="numeric ${c==='PER'?'lab-peru-col':''}">${esc(label(c))}</th>`).join('')}</tr></thead><tbody>${METRICS.map(([code,title])=>`<tr><th scope="row">${title}<small>${esc(wb(countries[0],code)?.unit||'')}</small></th>${countries.map(c=>{const s=wb(c,code),o=obs(s,lensYear),d=compareAtDates(s,baselineYear,lensYear),money=code==='NY.GDP.PCAP.CD';return `<td class="numeric ${c==='PER'?'lab-peru-col':''}">${o?`<button class="link-button" data-detail="${esc(s.id)}">${fmt(o.value,money?0:2)}</button>`:'Sin dato'}${baselineYear?`<small>${d?signed(money?d.relative:d.absolute)+' '+esc(money?'% nominal':d.unit):'Sin ambos datos'}</small>`:''}</td>`}).join('')}</tr>`).join('')}</tbody></table></div></details>
      <details class="lab-scatter-disclosure" open><summary>Producción y bienestar</summary><div class="lab-scatter-controls lab-controls"><label>Indicador<select id="lab-development">${DEVELOPMENT.filter(([c])=>series().some(s=>s.sourceCode===c)).map(([c,n])=>`<option value="${c}" ${c===development?'selected':''}>${n}</option>`).join('')}</select></label><label>Año<select id="lab-development-year"></select></label><span id="lab-development-coverage" class="lab-coverage"></span></div><div id="lab-scatter-host"></div></details>
    </section>`;
  ...
  function renderDevelopment(countries) {
    ...
    opts.xAxis={type:'value',name:'PBI por habitante · US$ corrientes',nameLocation:'middle',nameGap:33,nameTextStyle:{color:css('--muted'),fontSize:11},axisLabel:{color:css('--muted'),formatter:n=>Math.abs(n)>=1000?fmt(n/1000,0)+' mil':fmt(n,0)},splitLine:{lineStyle:{color:css('--line'),type:'dashed'}}};
    opts.yAxis={type:'value',scale:true,name:yunit,nameTextStyle:{color:css('--muted'),fontSize:11},axisLabel:{color:css('--muted'),formatter:n=>fmt(n,Math.abs(n)>1000?0:1)},splitLine:{lineStyle:{color:css('--line'),type:'dashed'}}};
  ...
  function mountExplorer() {
    ...
    main.insertAdjacentHTML('afterbegin',`<section id="explorer-lab" class="explorer-lab panel" aria-label="Guardar análisis y elegir periodo"><div class="lab-section-heading"><div><h2>Mis comparaciones</h2></div><small>En este navegador</small></div><div class="lab-save-row"><label class="lab-save-name">Nombre<input id="lab-save-name" maxlength="60" placeholder="Ej. Inflación y política monetaria"></label><button id="lab-save">Guardar</button><label class="lab-saved-select">Mis comparaciones<select id="lab-saved"><option value="">${saved.length?'Elegir comparación ('+saved.length+')':'Aún no hay comparaciones'}</option>${saved.map(x=>`<option value="${esc(x.id)}" ${x.id===chosenSaved?'selected':''}>${esc(x.name)}</option>`).join('')}</select></label><button id="lab-delete" ${chosenSaved?'':'disabled'} aria-label="Eliminar la comparación guardada seleccionada">Eliminar</button></div><div class="lab-date-row"><label>Desde<input type="date" id="lab-start" value="${start}"></label><label>Hasta<input type="date" id="lab-end" value="${end}"></label><button id="lab-window">Aplicar fechas</button><button id="lab-window-clear">Restablecer</button><span id="lab-window-note" role="status">${start||end?'Fechas aplicadas al historial seleccionado.':''}</span></div></section>`);
  ...
  function updateExplorerEvidence(groups,mode='level') {
    ...
    el.innerHTML=`<section class="lab-correlation panel"><div class="lab-section-heading"><div><h2>Correlaciones</h2><p>Pearson en fechas coincidentes. Selecciona un par para ver su dispersión.</p></div></div><div class="lab-series-key">${valid.map((g,i)=>`<span><b>${i+1}</b>${esc(safeName(g.s))}</span>`).join('')}</div><div class="lab-correlation-grid" style="--lab-count:${valid.length}" role="table" aria-label="Matriz de correlaciones"><div role="row" class="lab-correlation-row"><span role="columnheader">Serie</span>${valid.map((g,i)=>`<span role="columnheader" title="${esc(safeName(g.s))}">${i+1}</span>`).join('')}</div>${matrix.map((row,i)=>`<div role="row" class="lab-correlation-row"><span role="rowheader" title="${esc(safeName(valid[i].s))}">${i+1}</span>${row.map(cell=>{
    ...
    }).join('')}</div>`).join('')}</div><p class="lab-method">Mínimo: 12 pares y varianza no nula. Consulta fechas y cobertura al señalar cada celda. ${mode==='level'||mode==='index'?'Las tendencias comunes pueden elevar la correlación en niveles. ':''}Asociación no implica causalidad.</p></section>`;
