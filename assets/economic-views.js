import {monthlyRollingMeanRows,monthlyAnnualizedRows,spreadRows} from './chart-engine.js';
...
export function createEconomicViews(ctx) {
  ...
  function annualized(s,months,name) {
    return present(s)&&s.frequency==='monthly'?{...s,name,unit:'% anualizado',derived:`annualized${months}`,
  ...
  function topic(title,description,specs) {
    ...
    target.innerHTML=`<div class="economic-intro"><div><h2>${esc(title)}</h2><p>${esc(description)}</p></div><div class="economic-coverage"><span>${ready.length} gráficos · ${ss.length} series</span>${retained?`<span>${retained} con última descarga válida</span>`:''}</div></div>`+
  ...
  function renderPeruTopic(name) {
    if(name==='actividad')return topic('Producción, sectores y expectativas','Distingue el crecimiento anual del impulso mensual y de las expectativas empresariales.',[
      ...
      times('pe-domestic-demand','Consumo e inversión: los motores internos','Perú · variación interanual real (%) · trimestre frente al mismo trimestre del año anterior',[y(b('PN02529AQ'),'Consumo privado real · interanual'),y(b('PN02533AQ'),'Inversión privada real · interanual')],{names:['Consumo privado','Inversión privada'],zero:true},'Cálculo sobre niveles a precios constantes de 2007: trimestre actual frente al mismo trimestre del año anterior. No es una tasa trimestral anualizada. Las series capturan componentes distintos de la demanda interna.'),
    ...
    if(name==='trabajo')return topic('Empleo y financiamiento','Coberturas explícitas: empleo formal nacional, mercado laboral de Lima y crédito del sistema financiero.',[
      ...
      times('pe-youth-work','La brecha del desempleo juvenil','Lima Metropolitana · tasas de desempleo (%) · trimestres móviles',seriesCodes(['PN38063GM','PN38066GM']),{names:['Desempleo total','Jóvenes de 14–24 años']},'La cobertura es Lima Metropolitana, no todo el país. La tasa juvenil usa como denominador la PEA de 14–24 años. Las observaciones son trimestres móviles superpuestos.'),
    ...
    if(name==='fiscal')return topic('Las cuentas del Estado','Ingresos, gasto, déficit y deuda tienen unidades y horizontes distintos: los paneles los separan.',[
      ...
      times('pe-tax-composition','Los principales motores de la recaudación','Gobierno central · suma móvil de 12 meses · millones de soles',[rolling12(b('PN02296FM'),'Impuesto a la renta · 12 meses'),rolling12(b('PN02301FM'),'IGV · 12 meses'),rolling12(b('PN02304FM'),'ISC · 12 meses')],{names:['Impuesto a la renta','IGV','ISC']},'Rubros tributarios seleccionados; no suman toda la recaudación. Las devoluciones se publican por separado. Se usan 12 meses completos para reducir el efecto del calendario tributario.'),
      times('pe-tax-cycle','La recaudación y el ciclo de ingresos','Gobierno general · variación interanual nominal (%)',[y(b('PN02205FM'),'Ingresos tributarios · interanual'),y(b('PN02204FM'),'Ingresos corrientes · interanual')],{names:['Ingresos tributarios','Ingresos corrientes'],zero:true},'Compara cada flujo con el mismo mes del año anterior. Las variaciones son nominales: incluyen precios, actividad, cambios normativos y pagos extraordinarios; no equivalen al crecimiento de la base tributaria.'),
    ...
    if(name==='externo')return topic('Perú y el resto del mundo','Exportaciones, saldo comercial, reservas y precios relativos: canales de transmisión de la economía global.',[
      ...
      times('pe-export-import-prices','El canal de precios del comercio exterior','Precios de exportación e importación · índices 2007=100',seriesCodes(['PN38915BM','PN38919BM']),{names:['Precios de exportación','Precios de importación']},'Ambos índices comparten año base; se comparan trayectorias, no precios en dólares. Su cociente determina los términos de intercambio. Un aumento de precios no implica por sí solo mayor volumen de comercio.'),
    ...
    if(name==='mercado')return topic('Tasas de interés y moneda','Conecta decisiones monetarias, financiamiento soberano y mercado cambiario; cada instrumento mantiene su definición.',[
      times('pe-policy-transmission','De la tasa de referencia al mercado monetario','Perú · porcentaje anual · datos mensuales',seriesCodes(['PD04722MM','PN07819NM','PN07809NM']),{names:['Referencia BCRP','Interbancaria en soles','Corporativa 90 días en soles']},'Las tasas corresponden a instrumentos, promedios y riesgos diferentes. La tasa preferencial corporativa no representa el costo de crédito para todas las empresas.'),
      times('pe-sovereign-curves','El costo soberano por moneda','Bonos del Perú a 10 años · porcentaje anual',seriesCodes(['PD31895MM','PD31896MM']),{names:['Bono en soles','Bono en dólares']},'Los rendimientos en monedas distintas incorporan inflación, riesgo cambiario, liquidez y condiciones de cada mercado. Su diferencia no equivale al riesgo país ni a una expectativa cambiaria pura.'),
      times('pe-fx-history','El sol frente al dólar','Tipo de cambio interbancario promedio · soles por US$',[b('PN01207PM')],{names:['Soles por dólar'],area:true},'Un aumento indica depreciación del sol frente al dólar; una caída, apreciación. Es un promedio mensual interbancario, no una cotización de compra o venta disponible en este momento.'),
      times('pe-fx-rate-gap','El diferencial de tasas corporativas por moneda','Preferencial corporativa a 90 días · soles menos dólares · pp',[difference(b('PN07809NM'),b('PN07829NM'),'Diferencial corporativo soles − dólares')],{names:['Diferencial de tasas'],zero:true},'Diferencia exacta de las dos tasas promedio en un mismo mes. No incorpora la variación del tipo de cambio, seguros ni el costo de cubrir el riesgo cambiario.')
    ]);
  ...
  function renderWorldTopic(name) {
    if(name==='actividad')return topic('El ciclo de las grandes economías','Compara el crecimiento mundial anual con señales más frecuentes de Estados Unidos, sin mezclar sus horizontes.',[
      ...
      times('world-transatlantic-cycle','EE. UU. y la zona euro de 19 países','PBI real desestacionalizado · variación interanual (%)',[y(f('GDPC1'),'PBI real EE. UU. · interanual'),y(f('CLVMNACSCAB1GQEA19'),'PBI real zona euro 19 · interanual')],{names:['Estados Unidos','Zona euro · 19 países'],zero:true},'Trimestre actual frente al mismo trimestre del año anterior, a precios constantes. La serie europea mantiene una composición fija de 19 países, que no incluye todas las incorporaciones posteriores a la zona euro. Aunque los niveles originales usan monedas distintas, aquí se comparan sus tasas; no su tamaño económico.'),
      times('us-broad-activity','Una lectura amplia de la actividad de EE. UU.','Chicago Fed CFNAI · media móvil de 3 meses',[f('CFNAIMA3')],{names:['Actividad nacional CFNAI-MA3'],zero:true,recession:f('USREC')},'El índice resume múltiples indicadores de actividad. Cero representa el ritmo de crecimiento tendencial histórico; un valor negativo indica crecimiento por debajo de esa tendencia, no necesariamente una caída del PBI.'),
    ...
    if(name==='precios')return topic('Inflación y política monetaria','Separa inflación anual, impulso reciente y compensación de mercado. Cada panel muestra su horizonte.',[
      times('world-consumer-inflation','La inflación en las grandes economías','Precios al consumidor · variación anual (%) · Banco Mundial',['USA','CHN','DEU','IND','WLD'].map(c=>wb(c,'FP.CPI.TOTL.ZG')),{names:['EE. UU.','China','Alemania','India','Mundo'],zero:true},'Comparación anual de la misma familia estadística. Las canastas y metodologías nacionales difieren. El agregado mundial sigue la metodología del Banco Mundial; no es el promedio simple de estas economías.'),
      times('us-inflation-pce','El indicador de precios que sigue la Fed','EE. UU. · PCE general y subyacente · variación interanual (%)',[y(f('PCEPI'),'PCE general · interanual'),y(f('PCEPILFE'),'PCE subyacente · interanual')],{names:['PCE general','PCE sin alimentos y energía'],referenceLines:[{value:2,label:'Objetivo de largo plazo · PCE general: 2%'}],recession:f('USREC')},'Variación del índice frente al mismo mes del año anterior. El <a href="https://www.federalreserve.gov/faqs/economy_14400.htm" target="_blank" rel="noopener noreferrer">objetivo de la Fed</a> se refiere a la inflación PCE general a largo plazo; no a mantener cada observación mensual exactamente en 2%.'),
      times('us-inflation-momentum','¿La inflación reciente pierde impulso?','PCE subyacente desestacionalizado · ritmos anualizados de 3, 6 y 12 meses',[annualized(f('PCEPILFE'),3,'PCE subyacente · 3 meses anualizado'),annualized(f('PCEPILFE'),6,'PCE subyacente · 6 meses anualizado'),annualized(f('PCEPILFE'),12,'PCE subyacente · 12 meses')],{names:['3 meses anualizado','6 meses anualizado','12 meses'],lineStyles:['solid','dashed','dotted'],zero:true,stats:false},'Cálculo: ((índice actual / índice de hace n meses)^(12/n) − 1) × 100. Los tres ritmos usan el mismo índice desestacionalizado. La anualización expresa qué pasaría si ese ritmo persistiera; no es una previsión.'),
      times('us-cpi-composition','IPC general frente al componente subyacente','EE. UU. · variación interanual (%) · datos desestacionalizados',[y(f('CPIAUCSL'),'IPC general · interanual'),y(f('CPILFESL'),'IPC subyacente · interanual')],{names:['IPC general','IPC sin alimentos y energía'],zero:true,recession:f('USREC')},'El IPC y el PCE usan coberturas y ponderaciones diferentes. Por eso este panel no sustituye al PCE ni aplica al IPC el objetivo de 2% de la Fed.'),
      times('eurozone-inflation','Inflación en la zona euro de 19 países','Índice armonizado de precios al consumidor · variación interanual (%)',[y(f('CP0000EZ19M086NEST'),'IPCA zona euro 19 · interanual')],{names:['IPCA zona euro · 19 países'],zero:true},'Variación interanual del índice armonizado no desestacionalizado, con una composición de 19 países; no incluye todas las incorporaciones posteriores a la zona euro. Su cobertura y metodología difieren del IPC y del PCE estadounidenses; la comparación internacional exige mantener esas diferencias presentes.'),
      times('world-inflation-compensation','Lo que descuenta el mercado de bonos','Inflación implícita Treasury/TIPS · porcentaje anual',fredCodes(['T5YIE','T10YIE']),{names:['Breakeven 5 años','Breakeven 10 años']},'La diferencia entre rendimientos nominales y reales incorpora expectativas de inflación, primas de riesgo y liquidez. No es una previsión pura ni la inflación ya observada.')
    ]);
    ...
    if(name==='liquidez')return topic('Tasas, dinero y liquidez global','Del balance de la Reserva Federal al costo hipotecario: distintas medidas de las condiciones financieras.',[
      times('world-curve-slopes','Las señales de la curva del Tesoro','EE. UU. · rendimiento largo menos rendimiento corto · puntos porcentuales',fredCodes(['T10Y2Y','T10Y3M']),{names:['10 años − 2 años','10 años − 3 meses'],zero:true,recession:f('USREC')},'Una lectura negativa indica inversión de la curva. Históricamente se ha asociado con riesgos del ciclo, pero no fija la fecha de una recesión ni ofrece una probabilidad por sí sola.'),
