...
export function regionalSeries(series, indicator, mode = 'level') {
  ...
  const formula = mode === 'ytd' ? indicator.kind === 'flow'
    ...
    : mode === 'mom' ? 'MOM: mes seleccionado frente al mes calendario anterior.' : 'YOY: mes seleccionado frente al mismo mes del año anterior.';
  return { ...series, id: `${series.id}_${mode}`, name: `${series.name} · ${mode.toUpperCase()}`, unit: '%', derived: mode,
    ...
    observations: series.observations.map(o => ({ date: o.date, value: regionalValue(series, indicator, o.date, mode) })) };
}
...
export function createRegionalView(ctx) {
  ...
  const showPeriod = d => date(d, 'monthly');
  ...
  function releaseCharts() {
    ...
    ['regional-map', 'regional-rank', 'regional-history'].forEach(id => chartRows.delete(id));
  }
  ...
  function kpis() {
    return `<section class="regional-references" aria-label="Indicadores de referencia de ${esc(region(regionId).name)}">${dataset.indicators.map(i => {
  ...
  async function render(target = '#economic-topic') {
    ...
    const ind = indicator(), periods = regionalPeriods(dataset.regions, dataset.series, ind, mode);
    ...
    el.innerHTML = `<div class="regional-controls"><label>Región<select id="regional-region">${dataset.regions.map(r => `<option value="${r.id}" ${r.id === regionId ? 'selected' : ''}>${esc(r.name)}</option>`).join('')}</select></label><label>Indicador<select id="regional-indicator">${dataset.indicators.map(i => `<option value="${i.id}" ${i.id === indicatorId ? 'selected' : ''}>${esc(i.name)}</option>`).join('')}</select></label><label>Periodo común<select id="regional-period" ${periods.length ? '' : 'disabled'}>${periodOptions(periods)}</select></label><label>Vista<select id="regional-mode"><option value="level" ${mode === 'level' ? 'selected' : ''}>Nivel</option><option value="yoy" ${mode === 'yoy' ? 'selected' : ''}>YOY</option><option value="mom" ${mode === 'mom' ? 'selected' : ''}>MOM</option><option value="ytd" ${mode === 'ytd' ? 'selected' : ''}>YTD</option></select></label></div>
      <div class="regional-section-head"><h2>${esc(regionalName)} <span>· ${stamp}</span></h2><span>Indicadores de referencia</span></div>${kpis()}
      <div class="regional-summary" aria-live="polite"><div><strong>${esc(ind.name)}</strong><span>${mode === 'level' ? esc(ind.unit) : mode.toUpperCase() + ' · %'} · ${stamp}</span></div><div class="regional-selected-value"><b>${finite(chosen?.value) ? fmt(chosen.value, 1) : 'Sin dato'}</b><span>${finite(chosen?.value) ? `${esc(unit(ind))} · ${rank} de ${valid.length} con dato` : esc(regionalName)}</span></div><span class="regional-coverage">${valid.length}/${dataset.regions.length} regiones</span></div>
      ...
      <details class="regional-method"><summary>Fuentes y cálculo</summary><p>${esc(selectedSeries?.description || ind.description || '')}</p><dl><dt>YOY / MOM</dt><dd>Cambio porcentual frente al mismo mes del año anterior / mes anterior, con ambos datos publicados.</dd><dt>YTD</dt><dd>${ind.kind === 'flow' ? 'Acumulado de enero al mes seleccionado frente al mismo periodo del año anterior. Requiere todos los meses de ambos periodos.' : 'Saldo del mes seleccionado frente al saldo de diciembre del año anterior.'}</dd><dt>Cobertura</dt><dd>El mapa y la comparación usan un único periodo. Sin dato no equivale a cero. La ubicación estadística de una operación no mide el ingreso por habitante.</dd></dl><ul>${[...new Map([selectedSeries, peerSeries].filter(Boolean).map(s => [s.sourceUrl, s])).values()].map(s => `<li><a href="${esc(s.sourceUrl)}" target="_blank" rel="noopener">${esc(s.sourceInstitution || s.provider)}${s.sourceInstitution && s.sourceInstitution !== s.provider ? ' vía ' + esc(s.provider) : ''} · ${esc(s.sourceCode || s.name)}</a></li>`).join('')}</ul><p><a href="${esc(geometry.metadata.sourceUrl)}" target="_blank" rel="noopener">Cartografía: ANA · SNIRH</a> · límites referenciales. <span>Datos: ${new Intl.DateTimeFormat('es-PE', { dateStyle: 'medium', timeZone: 'America/Lima' }).format(new Date(dataset.fetchedAt))}.</span></p>${selectedSeries?.status === 'retained' ? '<p>Esta serie conserva la última descarga válida.</p>' : ''}</details>`;
    controller = new AbortController();
    el.addEventListener('change', event => {
      const { id, value } = event.target;
      if (id === 'regional-region') return selectRegion(value);
      if (id === 'regional-indicator') { indicatorId = value; period = ''; }
    ...
    el.addEventListener('click', event => {
      const button = event.target.closest('[data-regional-indicator],[data-regional-select],[data-map-zoom]');
      if (!button) return;
      if (button.dataset.regionalIndicator) { indicatorId = button.dataset.regionalIndicator; period = ''; return redraw(); }
  ...
  function drawMap(rows, ind) {
    ...
    opt.tooltip = { ...opt.tooltip, trigger: 'item', formatter: p => {
      const r = rows.find(r => r.name === p.name); return `${esc(p.name)}<br>${showPeriod(period)}<br><strong>${finite(r?.value) ? `${fmt(r.value, 2)} ${esc(unit(ind))}` : 'Sin dato'}</strong>`;
    }};
    ...
    chartRows.get('regional-map').rows = rows.filter(r => r.s).map(r => ({ s: regionalSeries(r.s, ind, mode), rows: [{ date: period, value: r.value }] }));
  ...
  function drawRanking(rows, ind) {
    ...
    chartRows.get('regional-rank').rows = rows.map(r => ({ s: regionalSeries(r.s, ind, mode), rows: [{ date: period, value: r.value }] }));
